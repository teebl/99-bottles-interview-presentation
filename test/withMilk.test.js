...
Take one down and pass it around, 7 bottles of milk on the wall.`;
    ...
    expect(verse(8, { drink: "milk" })).to.equal(expected);
  });
...
Take one down and pass it around, 5 bottles of milk on the wall.`;
    expect(sing(8, 6, { drink: "milk" })).to.equal(expected);
  });
...
Go to the store and buy some more, 99 bottles of milk on the wall.`;
    expect(sing(3, 0, { drink: "milk" })).to.equal(expected);
  });
});
